...
- Al crear un plan o mealprep familiar, un slug repetido no puede impedir el guardado ni sobrescribir el registro existente: `saveMenuItem` intenta la dirección solicitada y, si Supabase informa la colisión del índice `menu_items_slug_unique_idx`, guarda una variante libre con sufijo numérico (`-2`, `-3`, etc.). La UI informa el ajuste sin exponer el error técnico. Las ediciones de un registro existente conservan su validación normal para que una dirección elegida explícitamente no cambie de forma inesperada.
- El SKU automático de un formulario nuevo se mantiene dentro de su borrador local y remoto y funciona como identidad de continuidad entre dispositivos. Antes de insertar un plan o mealprep familiar sin `id`, `saveMenuItem` busca ese SKU: si ya corresponde a un registro guardado desde otro computador, actualiza ese registro y explica que retomó el borrador. Si dos equipos guardan casi al mismo tiempo, una colisión de SKU vuelve a consultar y actualiza la versión existente. No crear una segunda ficha por el mismo borrador.
...
- En un plan mensual, la pestaña `Mealpreps` permite dos recorridos equivalentes: sumar mealpreps individualmente desde la biblioteca o importar un menú semanal completo. La importación crea copias editables de todos los mealpreps del menú semanal, conserva sus vínculos con la biblioteca y guarda el nombre del menú de origen como referencia. No es una dependencia viva: editar ese menú semanal después no altera un mes ya armado; volver a importarlo incorpora otra copia de sus mealpreps.
...
- El orden comercial de `Planes` y `Mealpreps familiares` se mantiene en `menu_items.display_order`, pero no se administra con números: cada fila muestra su posición y permite subir, bajar o arrastrar desde un tirador. El cambio persiste de inmediato y sólo actualiza ese orden. Dentro de un plan, los mealpreps usan el mismo patrón; su secuencia se conserva en el borrador y se publica al guardar el plan.
