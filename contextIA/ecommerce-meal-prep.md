...
- La edición de mealpreps dentro de un plan abre siempre la `Ficha completa`: información, ingredientes, alérgenos, descripción nutricional, valores nutricionales, tags, beneficios e imágenes. La foto principal es obligatoria; la segunda es opcional. `Guardar mealprep` lo guarda en `Mealpreps` para reutilizarlo y `Guardar plan` sincroniza automáticamente todos sus mealpreps con esa biblioteca. Volver al plan conserva los cambios en su borrador.
