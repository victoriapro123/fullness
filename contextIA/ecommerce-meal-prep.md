...
- Recuperación 2026-07-27 desde el video de Cecilia: la migración crea para `cecilia.prueba@fullnesslab.com` el borrador no publicado `Meal prep semanal`, plan semanal, slug `meal-prep-semanal`, etiqueta `6 meal preps`, precio `$54.780`, orden `40`, con `Pollo, camote y cúrcuma` / `Energético`. Los campos que no se veían en el video quedan vacíos; nunca inventarlos ni activarlo como producto.
